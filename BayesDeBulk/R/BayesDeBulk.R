gibbs.sampling<-function(n.iter,Y,p,n,K,index.matrix,burn.in,mean.prior,sigma.prior){
  ...
  pi.mean<-mu.mean<-pi.mean[seq(1,K)]
  ...
  for (j in 1:n.iter){ # -- for loop over iterations
    ...
    for (k in 1:K) {
      ...
      if (sum(index.matrix[,2]==k | index.matrix[,1]==k)!=0){
        ...
        if (length(gene.higher) >= 1)  mean.final[gene.higher,k]<-rtruncnorm(1, a=bound, b=Inf, mean = mean.gene.h, sd = sqrt(var.gene.h))
...
BayesDeBulk<-function(n.iter,burn.in,Y,markers,prior=NULL){
  ...
  if (is.null(prior)) {prior.levels <- matrix(0,dim(data)[1],k.fix) } else {
    prior<-prior[,cellTypeNames]
    mg<-match(str_replace(str_replace(rownames(data)," -data1","")," -data2",""),rownames(prior))
    prior.levels<-prior[mg,]
  }
